# 13.4.4 Consulting Multiple Parameters: no version element - Test Case
# Client side.
...
def script(conn):
    ...
    assert_equals(len(msg.tcu_data.items[0].data), 0)
    assert_equals(msg.tcu_data.items[1].type, 0x19)
    assert_equals(len(msg.tcu_data.items[1].data), 0)
    assert_equals(msg.tcu_data.items[2].type, 0x51)
    assert_equals(len(msg.tcu_data.items[2].data), 0)
